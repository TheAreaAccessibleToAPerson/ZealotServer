...
namespace Zealot.client.listen._TCPShield
{
    public abstract class Controller : Butterfly.Controller
    {
        public const string NAME = "TCPListener";

        /// <summary>
        /// Задает место в которое будет перенаправлятся сообщения.
        /// </summary>
        private TCPShield.IListen _listen { set; get; } = null;
        ...
        /// к серверу.
        /// </summary>
        protected void Initialize(TCPShield.IListen listen)
        {
            Logger.S_I.To(this, $"{listen.GetKey()} Передал интерфейс для получения входящих tcp подключений.");
        ...
        private sealed class Content : content.Controller
        {
            void Construction()
            {
                ...
                {
                    send_impuls(ref I_startListener, TCPShield.BUS.Content.Impuls.START_LISTENER);
                    send_impuls(ref I_stopListener, TCPShield.BUS.Content.Impuls.STOP_LISTENER);
